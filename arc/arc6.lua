...
-- v2.2 2026-05-26 @rew62  moon phase split: text row at West/East y; icon+rise/set row below
...
local function draw_horizon_line(cr, cx, cy)
  ...
  if fc then
    for i = 1, math.min(2, #fc) do
      if fc[i].temp_high and not hi_n then hi_n = fc[i].temp_high end
      if fc[i].temp_low  and not lo_n then lo_n = fc[i].temp_low  end
    end
  end

  if cur_f and lo_n and hi_n then
    local bar_w  = rx - lx
