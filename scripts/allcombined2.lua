-- allcombined2.lua - Combines background, bar, and calendar drawing functions (v2)
-- by mrpeachy
...
function conky_draw_bg(bgtab)
    ...
    -- Title settings
    local title_font       = bgtab[12] or "Sans"
    local title_size       = bgtab[13] or 12
    local title_offset     = bgtab[14] or math.floor(title_size / 2)
    local gap_pad          = bgtab[15] or math.max(8, math.floor(title_size * 0.6))
    local title_text_offset = bgtab[16] or 7 -- moves text & patch vertically

    if w == 0 then w = conky_window.width end
    if h == 0 then h = conky_window.height end
    ...
    local fr,fg,fb,fa = rgb_to_r_g_b({color,alpha})
    local sr,sg,sb,sa = rgb_to_r_g_b({olcolor,olalpha})

    --#############################################################
    -- GET WINDOW TITLE
    local title = conky_title
    ...
    --#############################################################
    -- GEOMETRY
    local y_offset = title and title_offset or 0

    local xtl = x + r
    local ytl = y + r + y_offset

    local xtr = x + w - r
    local ytr = y + r + y_offset
    ...
    cairo_arc(cr, xtr, ytr, r, 3*math.pi/2, 2*math.pi)
    cairo_line_to(cr, xbr + r, ybr)
    cairo_arc(cr, xbr, ybr, r, 0, math.pi/2)
    cairo_line_to(cr, xbl, ybl + r)
    cairo_arc(cr, xbl, ybl, r, math.pi/2, math.pi)
    cairo_line_to(cr, xtl - r, ytl)
    cairo_arc(cr, xtl, ytl, r, math.pi, 3*math.pi/2)
    cairo_close_path(cr)
    ...
    --#############################################################
    -- DRAW BORDER (with split top)
    if draw == 2 or draw == 3 then
        cairo_set_operator(cr, CAIRO_OPERATOR_OVER)
        cairo_set_line_width(cr, lwidth)
        cairo_set_source_rgba(cr, sr,sg,sb,sa)
        cairo_new_path(cr) -- clear accumulated fill path so it isn't stroked

        -- Inset the border path by half the line width so the full stroke
        -- falls inside the fill bounds and doesn't clip at the window edge.
        local hw = lwidth / 2
        local ri = r - hw  -- inset corner radius

        local center_x = x + w / 2

        if title then
            local left_end = center_x - (text_w / 2) - gap_pad
            local right_start = center_x + (text_w / 2) + gap_pad
            local top_y = ytl - ri

            cairo_move_to(cr, xtl, top_y)
            cairo_line_to(cr, left_end, top_y)

            cairo_move_to(cr, right_start, top_y)
            cairo_line_to(cr, xtr, top_y)
        else
            cairo_move_to(cr, xtl, ytl - ri)
            cairo_line_to(cr, xtr, ytr - ri)
        end

        cairo_arc(cr, xtr, ytr, ri, 3*math.pi/2, 2*math.pi)
        cairo_line_to(cr, xbr + ri, ybr)
        cairo_arc(cr, xbr, ybr, ri, 0, math.pi/2)
        cairo_line_to(cr, xbl, ybl + ri)
        cairo_arc(cr, xbl, ybl, ri, math.pi/2, math.pi)
        cairo_line_to(cr, xtl - ri, ytl)
        cairo_arc(cr, xtl, ytl, ri, math.pi, 3*math.pi/2)

        cairo_stroke(cr)
    end

    --#############################################################
    -- BACKGROUND PATCH (moves with text)
    if title then
        local pad_x = gap_pad
        local pad_y = math.floor(title_size * 0.5)

        local box_x = x + (w / 2) - (text_w / 2) - pad_x
        local box_y = y + y_offset - r - pad_y/2 + title_text_offset
        local box_w = text_w + (pad_x * 2)
        local box_h = title_size + pad_y
    ...
    --#############################################################
    -- DRAW TITLE (moves independently with offset)
    if title then
        cairo_select_font_face(cr, title_font, CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_NORMAL)
        cairo_set_font_size(cr, title_size)
        cairo_set_source_rgba(cr, sr,sg,sb,sa)
        ...
        cairo_move_to(
            cr,
            x + (w / 2) - (ext.x_advance / 2),
            y + y_offset - 2 + title_text_offset
        )
